...
from ansible.module_utils.aws.s3 import dict_to_s3_extra_args
...
class S3Syncer(object):
    def __init__(self, module):
        ...
        try:
            ...
            module.fail_json_aws(e, msg="Couldn't connect to S3")

        self.objects = []

    def _calculate_etag(self, entry):
        if '-' in entry['s3']['e_tag']:
            # Multi-part ETag; a hash of the hashes of each part.
            parts = int(entry['s3']['e_tag'][1:-1].split('-')[1])
            digests = []
            with open(entry['path'], 'rb') as f:
                for part_num in range(1, parts + 1):
                    try:
                        key_head = self.s3.head_object(Bucket=self.bucket, Key=entry['key'], PartNumber=part_num)
                    except (BotoCoreError, ClientError) as e:
                        self.module.fail_json_aws(e, msg="Couldn't get metadata for multi-part object %s" % entry['key'])
                    digests.append(md5(f.read(int(key_head['ContentLength']))))
        ...
        return '"{0}"'.format(digest.hexdigest())
    ...
    def gather_s3_files(self):
        ...
        for s3_file in s3_files.get('Contents', []):
            ...
            if 'local' in entry and self.params['overwrite'] == 'different' and 'e_tag' in self.params['diff_attributes']:
                entry['local']['e_tag'] = self._calculate_etag(entry)
        ...
        for entry in self.objects:
            ...
            if not self.check_mode:
                ...
                if self.params['unsafe_writes']:
                    ...
                    self.module._module.atomic_move(tmp_file.name, entry['path'], unsafe_writes=self.params['unsafe_writes'])
...
def main():
    ...
    if not HAS_DATEUTIL:
        module.fail_json(msg='This module requires dateutil')

    if not HAS_MD5 and 'e_tag' in module.params['diff_attributes']:
