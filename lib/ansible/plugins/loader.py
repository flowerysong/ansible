...
from ansible.errors import AnsibleError, AnsiblePluginCircularRedirect, AnsiblePluginRemoved
...
class PluginLoader:
    ...
    def _query_collection_routing_meta(self, acr, plugin_type, extension=None):
        ...
        entry = collection_meta.get('plugin_routing', {}).get(plugin_type, {}).get(acr.resource + extension, None)
        if not entry:
            # try for extension-agnostic entry
            entry = collection_meta.get('plugin_routing', {}).get(plugin_type, {}).get(acr.resource, None)
        return entry
    ...
    def _find_fq_plugin(self, fq_name, extension, plugin_load_context):
        ...
        if routing_metadata:
            ...
            if redirect:
                ...
                display.warning("redirecting (type: {0}) {1} to {2}".format(plugin_type, fq_name, redirect))
    ...
    def find_plugin_with_context(self, name, mod_type='', ignore_deprecated=False, check_aliases=False, collection_list=None):
        ...
        # TODO: smuggle these to the controller, reduce noise from normal things like missing plugin packages during collection search
        ...
        if (AnsibleCollectionRef.is_valid_fqcr(name) or collection_list) and not name.startswith('Ansible'):
            ...
            for candidate_name in candidates:
                try:
                    ...
                    raise
                except AnsiblePluginCircularRedirect:
                    raise
...
def _on_collection_load_handler(collection_python_package_name):
    # parse collection metadata (if present) and set it under the package's _collection_meta
    # FUTURE: check collection metadata's declared Ansible version support string and warn/fail on mismatch
    metadata_package_name = collection_python_package_name + ".meta"
    collection_meta_raw = pkgutil.get_data(metadata_package_name, 'routing.yml')

    if not collection_meta_raw:
        collection_meta_raw = '{}'

    collection_package = sys.modules.get(collection_python_package_name)

    collection_meta = None

    try:
        collection_meta = yaml.load(collection_meta_raw, Loader=SafeLoader)
    except Exception as ex:
        display.warning("Error parsing collection metadata from {0}: {1}".format(metadata_package_name, ex))

    collection_package._collection_meta = collection_meta

    try:
        if not _does_collection_support_ansible_version(collection_meta.get('requires_ansible', ''), ansible_version):
            # TODO: configurable ignore/warn/fail?
            display.warning('Collection {0} does not support Ansible version {1}'.format(collection_package, ansible_version))
    except Exception as ex:
        display.warning('Error parsing collection metadata requires_ansible value from {0}: {1}'.format(metadata_package_name, ex))
...
def _does_collection_support_ansible_version(requirement_string, ansible_version):
    ...
    ss = SpecifierSet(requirement_string, prereleases=True)
    return ss.contains(ansible_version)


def _configure_collection_loader():
    if not any((isinstance(l, AnsibleCollectionLoader) for l in sys.meta_path)):
        loader = AnsibleCollectionLoader(C.config)
        loader.on_collection_load += _on_collection_load_handler
